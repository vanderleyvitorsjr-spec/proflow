...
import { formatBrazilianPhone, formatCep, formatCpfCnpj, normalizeProperName, onlyDigits } from "@/lib/br-formatters";
...
export function SupplierFormDialog({ open, supplier, onClose, onSave }: { open: boolean; supplier?: SupplierRecord; onClose: () => void; onSave: (value: SupplierFormValues, id?: string) => Promise<string | null>; }) {
  ...
  return <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/55" onKeyDown={(event) => { if (event.key === "Escape") onClose(); }}>
    <form className="h-full w-full max-w-3xl overflow-y-auto border-l bg-background shadow-2xl" onSubmit={async (event) => { event.preventDefault(); setSaving(true); setError(""); const result = await onSave(draft, supplier?.id); setSaving(false); if (result) setError(result); }}>
      ...
      <div className="space-y-6 p-5">
        ...
        <section><h3 className="text-sm font-semibold">Identificação</h3><div className="mt-3 grid gap-3 sm:grid-cols-2">
          <label className="text-xs font-medium">Razão social ou nome completo<Input ref={first} className="mt-1" value={draft.legalName} onChange={(e) => update("legalName", e.target.value)} onBlur={() => update("legalName", normalizeProperName(draft.legalName))} required /></label>
          <label className="text-xs font-medium">Nome fantasia ou exibição<Input className="mt-1" value={draft.tradeName} onChange={(e) => update("tradeName", e.target.value)} onBlur={() => update("tradeName", normalizeProperName(draft.tradeName))} required /></label>
          <label className="text-xs font-medium">CPF/CNPJ<Input className="mt-1" inputMode="numeric" value={formatCpfCnpj(draft.document)} onChange={(e) => update("document", onlyDigits(e.target.value).slice(0, 14))} placeholder="00.000.000/0000-00" /></label>
          <label className="text-xs font-medium">Contato principal<Input className="mt-1" value={draft.contactName} onChange={(e) => update("contactName", e.target.value)} onBlur={() => update("contactName", normalizeProperName(draft.contactName))} /></label>
        ...
        <section><h3 className="text-sm font-semibold">Contato</h3><div className="mt-3 grid gap-3 sm:grid-cols-2">
          <label className="text-xs font-medium">Telefone<Input className="mt-1" inputMode="tel" value={formatBrazilianPhone(draft.phone)} onChange={(e) => update("phone", onlyDigits(e.target.value).slice(0, 11))} placeholder="(00) 0 0000-0000" /></label>
          <label className="text-xs font-medium">WhatsApp<Input className="mt-1" inputMode="tel" value={formatBrazilianPhone(draft.whatsapp)} onChange={(e) => update("whatsapp", onlyDigits(e.target.value).slice(0, 11))} placeholder="(00) 0 0000-0000" /></label>
        ...
        <section><h3 className="text-sm font-semibold">Endereço</h3><div className="mt-3 grid gap-3 sm:grid-cols-6">
          <label className="text-xs font-medium sm:col-span-2">CEP<Input className="mt-1" inputMode="numeric" value={formatCep(draft.zipCode)} onChange={(e) => update("zipCode", onlyDigits(e.target.value).slice(0, 8))} /></label>
          <label className="text-xs font-medium sm:col-span-3">Logradouro<Input className="mt-1" value={draft.street} onChange={(e) => update("street", e.target.value)} /></label>
          ...
          <label className="text-xs font-medium sm:col-span-2">Bairro<Input className="mt-1" value={draft.district} onChange={(e) => update("district", e.target.value)} /></label>
          <label className="text-xs font-medium sm:col-span-2">Cidade<Input className="mt-1" value={draft.city} onChange={(e) => update("city", e.target.value)} onBlur={() => update("city", normalizeProperName(draft.city))} /></label>
          <label className="text-xs font-medium">UF<Input className="mt-1" maxLength={2} value={draft.state} onChange={(e) => update("state", e.target.value.toUpperCase())} /></label>
          <label className="text-xs font-medium">Complemento<Input className="mt-1" value={draft.complement} onChange={(e) => update("complement", e.target.value)} /></label>
        </div></section>
        ...
        <section><h3 className="text-sm font-semibold">Condições comerciais</h3><div className="mt-3 grid gap-3 sm:grid-cols-2">
          ...
          <label className="text-xs font-medium">Pedido mínimo (R$)<Input className="mt-1" type="number" min={0} step="0.01" value={(draft.minimumOrderCents ?? 0) / 100} onChange={(e) => update("minimumOrderCents", Math.round(Number(e.target.value) * 100))} /></label>
      ...
      <div className="sticky bottom-0 flex justify-end gap-2 border-t bg-background/95 px-5 py-4 backdrop-blur"><Button type="button" variant="secondary" onClick={onClose}>Cancelar</Button><Button disabled={saving}>{saving ? "Salvando..." : supplier ? "Salvar alterações" : "Cadastrar fornecedor"}</Button></div>
    </form>
  </div>;
}
