...
export const listQuotesAction = (filters?: Parameters<typeof quotesService.list>[0]) => quotesService.list(filters);
export const getQuoteAction = (id: string) => quotesService.get(id);
export const createQuoteAction = (input: Parameters<typeof quotesService.create>[0]) => quotesService.create(input);
export const updateQuoteAction = (id: string, changes: Parameters<typeof quotesService.update>[1]) => quotesService.update(id, changes);
export const duplicateQuoteAction = (id: string) => quotesService.duplicate(id);
export const newQuoteVersionAction = (id: string) => quotesService.newVersion(id);
export const transitionQuoteAction = (id: string, status: Parameters<typeof quotesService.transition>[1], details?: string) => quotesService.transition(id, status, details);
...
export const convertQuoteToOrderAction = async (
  ...
  const quote = await quotesService.get(quoteId);
  ...
  await quotesService.linkOrder(quoteId, order.id, order.orderNumber);
  return { quote: await quotesService.get(quoteId), order };
};
