...
import { compareQuoteVersions, describeQuotePaymentTerms, validateQuoteConversion, type ProfessionalQuote } from "./orcamentos-domain";
import { convertQuoteToOrderAction, getQuoteAction, listQuotesAction, newQuoteVersionAction, transitionQuoteAction } from "./orcamentos-actions";
...
export function OrcamentoVisualizacao({ quoteId }: { quoteId: string }) {
  ...
  useEffect(() => {
    void Promise.all([getQuoteAction(quoteId), listQuotesAction({ archived: false }), getDocumentIdentityAction()]).then(([current, all, company]) => {
  ...
  const document = quoteDocument({
    ...
    client: quote.clientName, clientDocument: quote.clientDocument, clientPhone: quote.clientPhone,
    clientEmail: quote.clientEmail, address: quote.address, serviceType: quote.serviceType,
    description: quote.description, items: quote.items, subtotalCents: quote.subtotalCents,
    discountCents: quote.discountCents + (quote.itemDiscountCents ?? 0), surchargeCents: quote.surchargeCents, totalCents: quote.totalCents,
    paymentTerms: describeQuotePaymentTerms(quote.totalCents, quote.paymentTerms), deadline: quote.executionDeadline, warranty: quote.warranty,
    notes: quote.customerNotes, terms: quote.terms,
  });
  async function convert() {
    if (!quote || conversionBusy) return;
    ...
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Não foi possível converter o Orçamento."); setConversionBusy(false); }
  }
  return <div className="space-y-3">
    <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border bg-card p-3 print:hidden">
      ...
      <div className="flex flex-wrap gap-2"><Button asChild size="sm" variant="secondary"><Link href={`/dashboard/orcamentos/${quote.id}/editar`}>Editar</Link></Button><Button size="sm" variant="secondary" onClick={() => newQuoteVersionAction(quote.id).then((created) => router.push(`/dashboard/orcamentos/${created.id}/editar`))}>Criar Nova Versão</Button>{["SENT", "VIEWED"].includes(quote.status) ? <Button size="sm" variant="secondary" onClick={() => transitionQuoteAction(quote.id, "APPROVED").then(setQuote)}>Aprovar Orçamento</Button> : null}{quote.serviceOrderId ? <Button asChild size="sm"><Link href={`/dashboard/ordens/${quote.serviceOrderId}`}>Abrir Ordem {quote.serviceOrderNumber}</Link></Button> : <Button size="sm" disabled={quote.status !== "APPROVED"} onClick={() => setConversion(true)}>Gerar Ordem de Serviço</Button>}</div>
    </div>
    {conversion ? <section className="rounded-xl border bg-card p-4 print:hidden">
      ...
      <div className="mt-3 flex gap-2"><Button size="sm" variant="secondary" disabled={conversionBusy} onClick={() => setConversion(false)}>Cancelar</Button><Button size="sm" disabled={Boolean(missing.length) || conversionBusy} onClick={() => void convert()}>{conversionBusy ? "Criando Ordem..." : "Confirmar e Criar Ordem"}</Button></div>
    </section> : null}
