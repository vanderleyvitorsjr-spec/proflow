...
const dayPeriodFormatter = new Intl.DateTimeFormat("pt-BR", {
  weekday: "long",
  day: "2-digit",
  month: "long",
  year: "numeric",
});
const typeLabels: Record<AgendaEventType, string> = {
...
function getPeriodLabel(view: AgendaView, date: Date) {
  if (view === "day") return dayPeriodFormatter.format(date);
  if (view === "week") {
...
function movePeriod(date: Date, view: AgendaView, direction: number) {
  const result = new Date(date);
  if (view === "day") result.setDate(result.getDate() + direction);
  else if (view === "week") result.setDate(result.getDate() + 7 * direction);
...
export function AgendaPageContent() {
  ...
  const [events, setEvents] = useState<AgendaDisplayEvent[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setLoadError("");
    try {
      setEvents(await listAgendaEventsAction());
    } catch (error) {
      setLoadError(
  ...
  useEffect(() => {
    void getAgendaConfiguration().then((result) => {
      setConfiguredTechnicians(result.team.map((item) => item.name));
      if (["day", "week", "month"].includes(result.settings.initialView))
        setView(result.settings.initialView as AgendaView);
    });
  }, []);
  const technicians = useMemo(
    () =>
      Array.from(
        new Set([...configuredTechnicians, ...events.map((event) => event.technician)]),
      ).sort((a, b) => a.localeCompare(b, "pt-BR")),
    [configuredTechnicians, events],
  );
  ...
  }, [events, searchTerm, technicianFilter, typeFilter]);
  const teams = useMemo<AgendaTeam[]>(
    () =>
      technicians.map((name) => {
        ...
        return {
          id: `team-${name.toLocaleLowerCase("pt-BR").replaceAll(" ", "-")}`,
          name,
          specialty: "Responsável da Agenda",
          status: active ? "BUSY" : "AVAILABLE",
        };
      }),
    [events, technicians],
  );
  const submit = async (value: AgendaEventFormValues) => {
    setSaving(true);
    setFormError("");
    try {
      await createAgendaEventAction(value);
      await load();
      setDrawer(false);
      setNotice(
        value.recurrence === "NONE"
          ? "Evento salvo com sucesso."
          : "Série recorrente salva com sucesso.",
      );
    } catch (error) {
      setFormError(
        error instanceof AgendaConflictError
          ? `${error.message} Conflitos: ${error.conflicts.map((item) => `${item.title} (${item.startAt.slice(11, 16)}–${item.endAt.slice(11, 16)})`).join(", ")}. Ajuste o horário ou o responsável.`
  ...
  const reschedule = async (eventId: string, date: string, time: string) => {
    setSaving(true);
    setNotice("");
    setLoadError("");
    try {
      await rescheduleAgendaEventAction(eventId, date, time);
      await load();
      setNotice("Compromisso reagendado com sucesso.");
    } catch (error) {
      setLoadError(
        error instanceof Error
          ? error.message
  ...
  const metrics = [
    ...
    {
      label: "Concluídos",
      value: filteredEvents.filter((e) => e.status === "COMPLETED").length,
      icon: CheckCircle2,
      tone: "success" as const,
    },
    {
      label: "Responsáveis disponíveis",
  ...
  return (
    <div className="space-y-3">
      <AgendaToolbar
        ...
        periodLabel={getPeriodLabel(view, currentDate)}
        ...
        onPreviousPeriod={() => setCurrentDate((date) => movePeriod(date, view, -1))}
        onNextPeriod={() => setCurrentDate((date) => movePeriod(date, view, 1))}
      ...
      <MetricStrip
        className="sm:min-w-0 sm:grid-flow-row sm:auto-cols-auto sm:grid-cols-2 xl:grid-cols-4"
      ...
      {loading ? (
        ...
        <section className="grid items-start gap-3 xl:grid-cols-[minmax(0,1fr)_18rem]">
          <div className="min-w-0">
            <AgendaCalendar
              ...
              onReschedule={
                saving ? undefined : (id, date, time) => void reschedule(id, date, time)
              }
            />
          </div>
          <AgendaSidebar
