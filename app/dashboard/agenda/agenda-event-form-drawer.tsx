    ...
    <section role="dialog" aria-modal="true" aria-labelledby="agenda-form-title" className="h-[100dvh] w-full overflow-hidden border-l border-border bg-background shadow-2xl sm:max-w-2xl">
      <form onSubmit={submit} className="flex h-full min-h-0 flex-col">
        ...
        <div className="grid flex-1 gap-4 overflow-y-auto p-4 pb-28 sm:grid-cols-2 sm:p-5">
          <Field className="sm:col-span-2" label="Compromisso ou serviço" description="Dê um nome objetivo ao evento. Ex.: Visita técnica na Empresa Solar." htmlFor="event-title" required><ProperNameInput id="event-title" autoFocus value={values.title} onValueChange={(value) => set("title", value)} /></Field>
          ...
          <div className="sm:col-span-2"><Label>Localização</Label><ProperNameInput value={values.address} onValueChange={(value) => set("address", value)} /></div><div><Label>Cidade</Label><ProperNameInput value={values.city} onValueChange={(value) => set("city", value)} /></div><div><Label>UF</Label><Input maxLength={2} value={values.state} onChange={(e) => set("state", e.target.value.toUpperCase())} /></div>
