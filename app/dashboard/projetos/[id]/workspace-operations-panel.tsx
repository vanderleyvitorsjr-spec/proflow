...
function MaterialSection({ serviceOrderId, items, run }: { serviceOrderId: string; items: WorkspaceOperationsEnvelope["materials"]; run: (fn: () => Promise<WorkspaceOperationsEnvelope>) => Promise<void> }) {
  const [stockItemId, setStockItemId] = useState(""), [name, setName] = useState(""), [quantity, setQuantity] = useState("1"), [cost, setCost] = useState("0,00");
  const [stockOptions, setStockOptions] = useState<Array<{ id: string; name: string; averageCostCents: number }>>([]);
  useEffect(() => { void listStockAction().then((result) => { if (result.ok) setStockOptions(result.data.map((snapshot) => ({ id: snapshot.item.id, name: snapshot.item.name, averageCostCents: snapshot.averageCostCents }))); }); }, []);
  return <section className="space-y-3"><FormGrid><Select value={stockItemId} onChange={(e) => { const id = e.target.value; setStockItemId(id); const item = stockOptions.find((option) => option.id === id); if (item) { setName(item.name); setCost((item.averageCostCents / 100).toFixed(2).replace(".", ",")); } }} aria-label="Item do Estoque"><option value="">Selecione no Estoque</option>{stockOptions.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</Select><Input value={quantity} onChange={(e) => setQuantity(e.target.value)} inputMode="decimal" placeholder="Quantidade Prevista" /><Input value={cost} onChange={(e) => setCost(e.target.value)} inputMode="decimal" placeholder="Custo Unitário em Reais" /><Button disabled={!stockItemId || !name.trim()} onClick={() => void run(() => addOrderMaterialAction({ id: `material-${crypto.randomUUID()}`, serviceOrderId, stockItemId, name, plannedQuantity: Number(quantity.replace(",", ".")), usedQuantity: 0, unit: "UNIT", unitCostCents: parseCurrencyBRToCents(cost), origin: "Estoque", status: "PLANNED", stockMovementConfirmed: false }))}><Plus className="h-4 w-4" />Adicionar Material</Button></FormGrid>{items.length ? <div className="divide-y rounded-xl border">{items.map((item) => <div key={item.id} className="grid gap-2 p-3 lg:grid-cols-[1fr_auto_auto] lg:items-center"><div><p className="text-sm font-semibold">{item.name}</p><p className="text-xs text-muted-foreground">{formatNumberBR(item.plannedQuantity, 2)} previsto(s) · {formatNumberBR(item.usedQuantity, 2)} utilizado(s) · {formatCurrencyBRLFromCents(materialTotalCents(item))}</p></div><Badge variant="outline">{ptBrLabel(item.status)}</Badge><Select value={item.status} aria-label={`Situação de ${item.name}`} onChange={(e) => { const status = e.target.value as typeof item.status; const confirmed = !["RESERVED", "USED", "RETURNED"].includes(status) || window.confirm("Confirmar a movimentação real no Estoque? Esta ação alterará saldos e ficará registrada."); void run(() => updateOrderMaterialStatusAction(item.id, status, confirmed)); }}><option value="PLANNED">Planejado</option><option value="RESERVED">Reservado</option><option value="USED">Utilizado</option><option value="PARTIALLY_USED">Parcialmente Utilizado</option><option value="RETURNED">Devolvido</option><option value="CANCELED">Cancelado</option></Select></div>)}</div> : <EmptyState size="compact" title="Nenhum Material Planejado" />}</section>;
}
