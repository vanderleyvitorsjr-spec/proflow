...
export const ordemSchema = z.object({
  clientId: z.string().min(1, "Selecione um cliente."), crmLeadId: z.string().optional().default(""), title: z.string().trim().min(3, "Informe o título."),
  description: z.string().trim().min(3, "Informe a descrição."), category: z.enum(["CLIMATIZATION", "ELECTRICAL", "PREVENTIVE", "CORRECTIVE", "INSTALLATION"]),
  ...
  technician: z.string().trim().min(2, "Informe o responsável."), address: z.string().trim().min(3, "Informe o endereço."), city: z.string().trim().min(2, "Informe a cidade."), state: z.string().trim().length(2, "Informe a UF."),
