...
const KEY = () => scopedBrowserStorageKey("equipamentos");
const BACKUP = () => scopedBrowserBackupKey("equipamentos");
...
export class LocalEquipmentStorageAdapter implements EquipmentStorageAdapter {
  async read() {
    ...
    copyLegacyBrowserDataToCompany("equipamentos");
...
class SyncedEquipmentStorageAdapter implements EquipmentStorageAdapter {
  private readonly local = new LocalEquipmentStorageAdapter();
  async read(): Promise<EquipmentStorageState> {
    try {
      const remote = await readRemoteModuleState<EquipmentStorageState>("equipamentos");
      if (remote.data && v3Schema.safeParse(remote.data).success) {
    ...
    try { await writeRemoteModuleState("equipamentos", local); } catch { /* mantém espelho */ }
    return local;
  }
  async write(state: EquipmentStorageState): Promise<EquipmentStorageState> {
    if (!v3Schema.safeParse(state).success) throw new EquipmentStorageError("O estado de Equipamentos é inválido e não foi salvo.");
    const next = { ...state, revision: state.revision + 1 };
...
export const equipmentStorageAdapter: EquipmentStorageAdapter = new SyncedEquipmentStorageAdapter();
