...
const pricingReference = (
  ...
  const currentDepreciation = depreciation(asset);
  const depreciationStarted = asset.depreciation.startDate
    ? new Date(`${asset.depreciation.startDate}T12:00:00`).getTime() <= Date.now()
    : false;
  const monthlyDepreciationCents =
    asset.depreciation.mode === "LINEAR" &&
    asset.depreciation.usefulLifeMonths &&
    depreciationStarted &&
    !currentDepreciation.fullyDepreciated
      ? Math.round(
      ...
      : 0;
  const maintenanceCutoff = Date.now() - 365 * 24 * 60 * 60 * 1000;
  const completed = maintenance.filter(
    (item) =>
      item.assetId === asset.id &&
      item.status === "COMPLETED" &&
      Boolean(item.completedAt) &&
      new Date(item.completedAt!).getTime() >= maintenanceCutoff,
  );
  return {
    id: asset.id,
    ...
    currentValueCents: currentDepreciation.currentValueCents,
