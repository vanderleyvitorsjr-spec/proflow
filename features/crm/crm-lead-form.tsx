  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <fieldset className="grid gap-3 md:grid-cols-2 xl:grid-cols-3"><legend className="col-span-full text-sm font-semibold">Informações da oportunidade</legend>
        ...
        <Field label="Prioridade do atendimento" htmlFor="lead-priority" error={errors.priority?.message} help="Use Alta ou Crítica quando o retorno comercial não puder esperar."><Select id="lead-priority" {...register("priority")}><option value="LOW">Baixa</option><option value="MEDIUM">Média</option><option value="HIGH">Alta</option><option value="CRITICAL">Crítica</option></Select></Field>
        ...
        <Field label="Etapa da negociação" htmlFor="lead-stage" error={errors.stageId?.message} help="Mostra em qual ponto do atendimento comercial esta oportunidade se encontra."><Select id="lead-stage" {...register("stageId")}>{crmStages.map((stage) => <option key={stage.id} value={stage.id}>{stage.title}</option>)}</Select></Field>
