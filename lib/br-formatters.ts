...
const UPPERCASE_TOKENS = new Set(["ltda", "mei", "epp", "sa", "s.a.", "cpf", "cnpj", "crea", "cau", "art", "pmoc", "hvac"]);
...
export function normalizeProperName(value: string | undefined | null): string {
  const cleaned = String(value ?? "").trim().replace(/\s+/g, " ").toLocaleLowerCase("pt-BR");
  if (!cleaned) return "";
  return cleaned
    .split(" ")
    .map((token, index) => {
      if (index > 0 && LOWERCASE_CONNECTORS.has(token)) return token;
      if (UPPERCASE_TOKENS.has(token)) return token === "s.a." ? "S.A." : token.toUpperCase();
      return token
        .split(/([-'])/)
        .map((part) => (/[-']/.test(part) ? part : part.charAt(0).toLocaleUpperCase("pt-BR") + part.slice(1)))
        .join("");
    })
    .join(" ");
}
...
export function formatNumberBR(value: number, minimumFractionDigits = 2): string {
  return new Intl.NumberFormat("pt-BR", { minimumFractionDigits, maximumFractionDigits: minimumFractionDigits }).format(value);
}
