    ...
    <article className="document-print mx-auto min-h-[277mm] max-w-[210mm] overflow-hidden bg-white px-8 py-7 text-[var(--document-text)] shadow-[0_18px_50px_rgba(15,23,42,0.12)] print:min-h-0 print:max-w-none print:overflow-visible print:p-0 print:shadow-none sm:px-11 sm:py-9">
      <header className="document-keep grid grid-cols-[minmax(0,1fr)_auto] gap-8 border-b border-[var(--document-border)] pb-6">
        <div className="min-w-0 pr-2">
          {data.identity?.logoUrl && !logoFailed ? <Image src={data.identity.logoUrl} alt="Logotipo da empresa" width={260} height={104} unoptimized onError={() => setLogoFailed(true)} className="mb-4 h-auto max-h-[104px] w-auto max-w-[260px] object-contain object-left" /> : <p className="mb-3 text-xl font-extrabold tracking-tight" style={{ color: accent }}>{data.identity?.companyName || data.identity?.legalName || "Empresa prestadora"}</p>}
